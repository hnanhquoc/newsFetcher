...
function CreateMenu(path){
	...
	GetWebsCategories(path).forEach(function(category){
		list += '<li><a style="cursor:pointer;color: white" onclick="GetRss(this.id)" id="'+category[0]+'" href=?url="'+category[2]+'">'+category[1]+'</a></li>';
	});
	
	document.getElementById("menu").innerHTML = list;
	} catch(error){
		document.getElementById("menu").innerHTML = '<li><a style="cursor:pointer;color: white">Đang phát triển</a></li>';
	}		
}
